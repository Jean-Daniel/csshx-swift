...
void _WBAEPrintDebug(const AEDesc *desc, CFStringRef format, ...) {
  ...
  NSLogv((__bridge NSString *)format, args);
  va_end(args);
}
...
CFStringRef WBAEDescCopyDescription(const AEDesc *desc) {
  ...
  if (noErr == err) {
    str = CFStringCreateWithCString(kCFAllocatorDefault, *handle, kCFStringEncodingUTF8);
...
OSStatus WBAEBuildAppleEventWithTargetBundleID(CFStringRef bundleID, AEEventClass theClass, AEEventID theID, AppleEvent *outEvent,
  ...
  va_list args;
  va_start(args, paramsFmt);
  pid_t pid = [NSRunningApplication runningApplicationsWithBundleIdentifier:(__bridge NSString *)bundleID].firstObject.processIdentifier;
...
#pragma mark Record
OSStatus WBAERecordCreate(AERecord *record, ...) {
  va_list args;
  va_start(args, record);
  OSStatus err = WBAERecordCreateWithArguments(record, args);
  va_end(args);
  return err;
}
OSStatus WBAERecordCreateWithArguments(AERecord *record, va_list args) {
  if (!record)
    return paramErr;
  OSStatus err = AECreateList(NULL, 0, TRUE, record);
  if (noErr == err) {
    err = WBAERecordAppendWithArguments(record, args);
    if (noErr != err)
      WBAEDisposeDesc(record);
  }
  return err;
}

OSStatus WBAERecordAppend(AERecord *record, ...) {
  va_list args;
  va_start(args, record);
  OSStatus err = WBAERecordAppendWithArguments(record, args);
  va_end(args);
  return err;
}
OSStatus WBAERecordAppendWithArguments(AERecord *record, va_list args) {
  if (!record)
    return paramErr;
  AEKeyword key;
  const AEDesc *desc;
...
struct PerThreadStorage {
  ...
  PerThreadStorage() : port(MACH_PORT_NULL) {
    ...
    mach_port_construct(mach_task_self(), &opts, 0, &port);
  }
  ~PerThreadStorage() {
