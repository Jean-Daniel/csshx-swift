...
final class WindowLayoutManagerTests: XCTestCase {
  ...
  func testSimpleLayout() throws {
    var (rows, columns) = getBestLayout(for: 16.0 / 9.0, hosts: 23, on: CGRect(x: 0, y: 0, width: 1024, height: 768))
    XCTAssertEqual(6, rows)
    XCTAssertEqual(4, columns)

    (rows, columns) = getBestLayout(for: 3.0 / 2.0, hosts: 23, on: CGRect(x: 0, y: 0, width: 1024, height: 768))
    XCTAssertEqual(5, rows)
    XCTAssertEqual(5, columns)
  }

  func testSingleHost() throws {
    let (rows, columns) = getBestLayout(for: 16.0 / 9.0, hosts: 1, on: CGRect(x: 0, y: 0, width: 1024, height: 768))
